...
**Typography and colour.** IBM Plex Sans 400 and 700 (Regular and Bold are what is vendored), embedded as two
latin-subset woff2 data URIs (base64 constants generated by `scripts/build-font-module.mjs` into
...
- **Document**: the file contains two `@font-face` rules with `data:` URIs
  ...
  graph.
- **Player**: the repo has no jsdom, so the player's logic (reducer, viewport maths, focus sets) is unit-tested as pure modules and the card itself is verified live. (Superseded: a component test with a mocked `fgRef` asserting `centerAt`
