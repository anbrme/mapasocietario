...
describe('presenter markup', () => {
  it('the card carries a Presentar button and the walkthrough a slide zero built from the opening', () => {
    ...
    expect(fig).toContain('<button id="wt-present" title="Flechas o espacio para avanzar · Esc para salir">Presentar</button>');
    expect(fig).toMatch(/<div class="wt-tools hide-print"><button id="wt-print">Imprimir<\/button><button id="wt-share" title="[^"]+">Compartir<\/button><button id="wt-save">Guardar copia<\/button>/);
    // The tools sit in their own row, after the navigation row.
    expect(fig.indexOf('class="wt-tools')).toBeGreaterThan(fig.indexOf('id="wt-exit"'));
