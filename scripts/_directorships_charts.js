/**
 * Interactivity for the Madrid directorships study. Inlined into the generated
 * page: plain browser JS, no build step, no external library. Every label comes
 * from the #study-data payload, so this file carries no copy and serves both
 * languages unchanged.
 *
 * One piece of state — the selected year — drives the concentration chart, the
 * connectivity bars, the province table and the explorer readout.
 */
(function () {
  'use strict';
  ...
  var L = D.labels;
  var ES = D.lang === 'es';
  var FIRST = D.first, LAST = D.last, N = D.panel.length - 1;
  ...
  function niceTicks(max, m) {
    var ladder = COUNTS[m] ? [1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000]
  ...
  /* ---------- line chart (shared by the concentration figure and the explorer) ---------- */
  function lineChart(cfg) {
    ...
    function draw() {
      ...
      var m = cfg.metric();
      var all = [];
      series.forEach(function (s) { D.panel.forEach(function (r) { all.push(r[s.key][m]); }); });
      ...
      [0, Math.round(N / 4), Math.round(N / 2), Math.round(3 * N / 4), N].forEach(function (i) {
        ax.appendChild(el('text', { x: x(i), y: H - 10, 'text-anchor': 'middle' }, String(D.panel[i].year)));
      });
      svg.appendChild(ax);
      var active = Math.max(0, Math.min(N, (hover == null ? year : hover) - FIRST));
      var row = D.panel[active];
      var area = 'M ' + x(0) + ' ' + y(0) + ' ' + D.panel.map(function (r, i) { return 'L ' + x(i) + ' ' + y(r[series[0].key][m]); }).join(' ') + ' L ' + x(N) + ' ' + y(0) + ' Z';
      svg.appendChild(el('path', { d: area, fill: series[0].color, opacity: '.08' }));
      series.forEach(function (s, j) {
        var d = D.panel.map(function (r, i) { return (i ? 'L ' : 'M ') + x(i) + ' ' + y(r[s.key][m]); }).join(' ');
        svg.appendChild(el('path', { d: d, fill: 'none', stroke: s.color, 'stroke-width': j ? 2 : 3, 'stroke-dasharray': j ? '5 6' : '', 'stroke-linejoin': 'round', 'stroke-linecap': 'round' }));
      });
      svg.appendChild(el('line', { x1: x(active), x2: x(active), y1: mt, y2: H - mb, stroke: css('--rule'), 'stroke-dasharray': '3 4' }));
      series.forEach(function (s) {
        svg.appendChild(el('circle', { cx: x(active), cy: y(row[s.key][m]), r: 10, fill: s.color, opacity: '.15' }));
        svg.appendChild(el('circle', { cx: x(active), cy: y(row[s.key][m]), r: 4.5, fill: s.color, stroke: css('--raise'), 'stroke-width': 2 }));
      });
      clear(readout);
      var yr = document.createElement('span'); yr.className = 'eyebrow'; yr.textContent = String(row.year); readout.appendChild(yr);
      series.forEach(function (s) {
        var v = document.createElement('span'); v.className = 'rv'; v.style.color = s.color;
        var b = document.createElement('b'); b.textContent = fmt(row[s.key][m], m); v.appendChild(b);
        var sm = document.createElement('small'); sm.textContent = s.label; v.appendChild(sm);
        readout.appendChild(v);
      });
      if (cfg.after) cfg.after(row, m, series);
    }
  ...
  function setYear(y) {
    ...
    document.querySelectorAll('.year-out').forEach(function (n) { n.textContent = String(y); });
  ...
  /* ---------- 1. concentration: B vs BX ---------- */
  charts.push(lineChart({
    svg: 's-conc', readout: 'r-conc',
    series: function () { return [{ key: 'B', label: L.variants.B, color: css('--stamp') }, { key: 'BX', label: L.variants.BX, color: css('--ink-3') }]; },
  ...
  playBtn.addEventListener('click', function () {
    if (playing) { clearInterval(playing); playing = null; playBtn.setAttribute('aria-pressed', 'false'); playBtn.textContent = L.play; return; }
    if (year === LAST) setYear(FIRST);
    playBtn.setAttribute('aria-pressed', 'true'); playBtn.textContent = L.pause;
    playing = setInterval(function () {
      if (year >= LAST) { clearInterval(playing); playing = null; playBtn.setAttribute('aria-pressed', 'false'); playBtn.textContent = L.play; return; }
      setYear(year + 1);
    }, 900);
  });
  ...
  /* ---------- 2. connectivity bars ---------- */
  var netVariant = 'B';
  document.querySelectorAll('#net-tabs [role=tab]').forEach(function (b) {
    b.addEventListener('click', function () {
      netVariant = b.getAttribute('data-k');
      document.querySelectorAll('#net-tabs [role=tab]').forEach(function (o) { o.setAttribute('aria-selected', o === b ? 'true' : 'false'); });
      drawBars();
    });
  });
  function drawBars() {
    ...
    years.forEach(function (yr) {
      var r = D.panel[yr - FIRST][netVariant];
      ...
      var sm = document.createElement('small'); sm.textContent = yr === FIRST ? L.start : L.selected; h.appendChild(sm);
      block.appendChild(h);
  ...
  document.querySelectorAll('#prov-head button').forEach(function (b, i) {
    b.addEventListener('click', function () {
      var k = KEYS[i];
  ...
  function provRows() {
    ...
    var n = D.national[year - FIRST], n0 = D.national[0];
    var spain = null;
    if (n && n0) spain = { name: L.spainRow, small: false, spain: true, companies: n.B.companies, top1: n.B.top1pct, delta: n.B.top1pct - n0.B.top1pct, gini: n.B.gini, interlocked: n.B.interlocked, giant: n.B.giant, ratio: n.B.giant / (n.B.giant_rnd_mean || 1), closure: null };
    rows.sort(function (a, b) {
      if (sortKey === 'name') return sortDir * a.name.localeCompare(b.name, ES ? 'es' : 'en');
      return sortDir * (a[sortKey] - b[sortKey]);
    });
    return spain ? [spain].concat(rows) : rows;
  }
  ...
  function drawProvinces() {
    ...
    document.querySelectorAll('#prov-head th').forEach(function (th, i) {
      th.setAttribute('aria-sort', KEYS[i] === sortKey ? (sortDir === 1 ? 'ascending' : 'descending') : 'none');
    });
  ...
  /* ---------- 4. explorer ---------- */
  var selV = document.getElementById('ex-variant'), selM = document.getElementById('ex-metric');
  var explorer = lineChart({
    svg: 's-ex', readout: 'r-ex',
    series: function () { return [{ key: selV.value, label: L.variants[selV.value], color: css('--stamp') }]; },
    metric: function () { return selM.value; },
    after: function (row, m) {
      var first = D.panel[0][selV.value][m], cur = D.panel[year - FIRST][selV.value][m];
      ...
      document.getElementById('ex-title').textContent = L.metrics[m] + ' · ' + L.variants[selV.value];
    },
  });
  charts.push(explorer);
  selV.addEventListener('change', function () { explorer.draw(); drawExplorerTable(); });
  selM.addEventListener('change', function () { explorer.draw(); drawExplorerTable(); });
